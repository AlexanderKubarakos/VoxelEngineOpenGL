...
Chunk::Chunk(glm::ivec3 t_ChunkPosition) : m_ChunkPosition{t_ChunkPosition}, m_BlockData{ new int8_t[32 * 32 * 32]() }, m_BucketIDs{nullptr}
{
	...
	fnGenerator->GenUniformGrid2D(noiseOutput.data(), t_ChunkPosition.x * 32, t_ChunkPosition.z * 32, 32, 32, 0.012f, 1337);
	allAir = true;
	for (int z = 0; z < 32; z++)
	{
		for (int y = 0; y < 32; y++)
		{
			for (int x = 0; x < 32; x++)
			{
				...
				if (noiseOutput[x + z * 32] * 32.0f > static_cast<float>(y + t_ChunkPosition.y * 32 - 32))
				{
					if (y + t_ChunkPosition.y * 32 > 16 - rand() % 2)
