...
void ChunkManager::ThreadedUnloadAndLoad(const Camera& camera)
{
	while (true)
	{
		...
		if (stopToken)
			break;
		lock.unlock();
		// TODO: maybe just lock after this loop and skip the conditional variable?
		while (!m_ChunksToAdd.empty() || !m_ChunksToRemove.empty())
		{
			
			// Wait till
			// BUG: IS HERE, we have the mutex locked, but we are waiting for the thread to process data that needs lock 
		}
		lock.lock();
		...
		std::deque<std::shared_ptr<Chunk>> loadQueue;
		...
		for (int x = -m_ViewDistance + playerPositionChunkSpace.x; x <= m_ViewDistance + playerPositionChunkSpace.x; x++)
			for (int z = -m_ViewDistance + playerPositionChunkSpace.z; z <= m_ViewDistance + playerPositionChunkSpace.z; z++)
				for (int y = -3; y <= 3; y++)
				{
					...
					if (!m_Chunks.contains(chunkPos) && loadQueue.size() < 512)
					{
...
void ChunkManager::ProcessChunks(const glm::vec3& t_PlayerPosition)
{
	...
	if ((!m_ChunksToRemove.empty() || !m_ChunksToAdd.empty() || !m_MeshDataToProcesses.empty()))
	{
		if (!lock.try_lock())
			return;
		TIMER_START(1)
